...
| **Next action** | **Cut `v0.9.1`.** Still the only way to prove **D5** end-to-end — auto-update code that has never performed an update is a hypothesis, not a tested path — and it now also ships the sound/voice feature to anyone who downloaded `v0.9.0`. Sequence: bump `version`, `npm run dist`, tag, publish, then let the *installed* `v0.9.0` discover it. After that: pick one of `LUNA_HUD_SPECIFICATION.md` §6's scored module ideas (session timeline scrubber, active-files heatmap, Spotify now-playing, terminal appearance customizer, …), or **E2/C2 panels**, **E4/C4 drag-drop**, or §9 multi-model. |
